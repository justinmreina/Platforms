...
static const char init[] = {0x40,    /*Display start line 0                    */
                            0xa1,    /*ADC reverse, 6 o'clock viewing direction*/
...
void halLcdPrintInt(char *pBuffer, int32 number,char x,char page){
  ...
  for(k=numOfDigits-1;k>=0;k--){
    ...
    for(j=0;j<LCD_FONT_WIDTH;j++)
    {
      *(pBuf + (firstPos + LCD_CHAR_WIDTH*k + j)) = alphabet[firstIndex+j];
    }
    *(pBuf + (firstPos + LCD_CHAR_WIDTH*k + LCD_FONT_WIDTH)) = 0x00;  /* Spacing */
...
void halLcdPrintFloat(char *pBuffer, float number, char numOfDecimals, char x,char page)
{
  ...
  uint8 i;
  for(i=0;i<numOfDecimals;i++)/* float must be to be considered negative.*/
...
void halLcdSetVLine(char *pBuffer, char x,char y_from,char y_to)
{
  ...
  char page;                                                                /* jmr added, this var wasn't declared??                */
...
void halLcdSendBufferAnimated(char *pToBuffer, char *pFromBuffer, char motion)
{
  ...
  char offset, pageIndex;
  uint16 i;
  for(offset = 0; offset<=LCD_COLS; offset+=4)
  {
    for(pageIndex=0; pageIndex<8; pageIndex++)   /* Page counter*/
    {
