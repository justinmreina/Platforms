...
Renderer.prototype.code = function(code, lang, escaped, caption) {
  ...
  if (!lang) {
    return '<div style="display:inline-block"><div style="display:block">'
      + '<button type="button" class="btn btn-xs btn-warning float-right" style="margin: 0; position: relative;"onclick="SelectText(\'codeBlock_'+codeBlockIdx+'\')">Select text</button></div>'
      + '<div class="pre-container"><pre><code id="codeBlock_'+codeBlockIdx+'">'
...
Renderer.prototype.callout = function(level, floating, title, text) {
  level = level
        .replace(/\[\[r/g, 'bs-callout-danger')
        .replace(/\[\[b/g, 'bs-callout-info')
        .replace(/\[\[g/g, 'bs-callout-success')
        .replace(/\[\[y/g, 'bs-callout-warning')
        .replace(/\[\[d/g, 'bs-callout-default')

  return '<div class="bs-callout '+level+' '+(floating ? 'float-right' : '') +'">'
    + '<h4>'+title+'</h4>\n'
    + text
    + '</div>\n'
};
...
Renderer.prototype.quiz = function(body) {
  ...
  while(match = /([XxVv])(>?) ([\S ]+)/.exec(body))
  {
    ...
    ret += '<label for="q' + qId + '" class="quiz-label '+(match[2]?'quiz-float':'')+'">'+match[3]+'</label>\n';
    questionNumber++;
  }
  ret += '</div></div>\n'
...
Renderer.prototype.image = function(href, title, text) {
  var out = '<a href="'+href+'"><img src="' + href + '" alt="' + text + '" class="img-responsive"';
  if (title) {
    out += ' title="' + title + '"';
  }
...
Parser.prototype.tok = function() {
  switch (this.token.type) {
    ...
    case 'callout_start': {
    	...
    	return this.renderer.callout(level, floating,
    		title,
    		text);
    }
    case 'alert_start': {
